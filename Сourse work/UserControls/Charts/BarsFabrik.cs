...
namespace WpfControlLibrary_BarDiagram
{
    ...
    class BarsFabrik
    {
        ...
        public BarsFabrik(UIElementCollection A, int count, double Max)
        {
            Col = A;
            BarsCount = count;
        ...
        public void CreateBarDiagram(List<double> ListData, List<string> StrMas, int n, int NumberColor)
        {
            ...
            CreateItemDiagram(ListData, StrMas, n, NumberColor);
        ...
        {
            Bar.BarLeftIndent = -1200;
            Bar.LeftDataIndent = -470;
        ...
        private void AlinementAxisX(List<double> ListData, int n)
        {
            ...
            if (MultiColumnDiagram == true)
            {
                ...
                else Bar.LeftDataIndent += 8 * (6 - BarsCount) + 15;
            }
        ...
        private void CreateItemDiagram(List<double> ListData, List<string> StrMas, int n, int NumberColor)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                TB = CreateTBData(ListData[i].ToString("#.##"));
                Col.Add(TB);
            }
...
//используется в ChartsAxis
        private void CreateIntervalAndOrientation(int n)
        {
            ...
            if (BarsCount != 1)
            {
                if (n <7)
                {
                    ...
                    LeftIntendantion -= (n == 6) ? 3 * (7 - n)-10 * BarsCount  : 10 * BarsCount;
                    int x = (BarsCount == 2) ? 3 : BarsCount;
                    Bar.LeftDataIndent += 15 * (x) - 5 * n;
                }
                else
                {
                    RectBarWidth -= 25;
                    Bar.BarLeftIndent -= 40 * BarsCount;
                    LeftIntendantion += 5 * BarsCount;
                }
            ...
            if (n < 6)
            {
                LeftIntendantion += 100 * (6 - n);
                RectBarWidth += 10 * (6 - n);
            }
            else
            {
                if (n < 11)
                { LeftIntendantion -= 15 * (n - 6); } //- 5 * BarsCount
                else
                {
                    if (n < 17)
                    {
                        Orientation = 340; //наклон подписей
                        LeftIntendantion -= 12 * (n - 6); //уменьшаем отступы
                        RectBarWidth -= 3 * (n - 10); //уменьшаем ширину столбца
                    }
                    else
                    {
                        Orientation = 320;
                        LeftIntendantion -= 132;
                        RectBarWidth -= 3 * (n - 10);
                    }
        ...
        private Rectangle CreateRect(int NumberColor)
        {
            ...
            Grid.SetColumn(TempRect, 1);
            Grid.SetColumnSpan(TempRect,2);
            return TempRect;
        }
        //используется в CreateItemDiagram
